...
public interface RegisterMapper {
    ...
    @Select("select * from doc_patient where id in (select user_ID from doc_register where doctor_Id = #{id}" +
            " and DATE = #{date,jdbcType=DATE} and work_time = #{workTime});")
    List<Patient> queryPatientByDoctorAndDate(@Param("id") int doctorId, @Param("date") Date date, @Param("workTime")int workTime);
